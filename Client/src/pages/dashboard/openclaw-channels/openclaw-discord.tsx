...
import type {
  ...
  OpenClawDiscordAccount,
  OpenClawDiscordConfig,
  OpenClawDiscordConfigUpdateRequest,
  OpenClawDiscordStatusResponse,
} from '@/api'
import {
  deleteOpenClawDiscordAccount,
  getOpenClawDiscordAddAccountStreamURL,
  getOpenClawDiscordStatus,
  listOpenClawAgents,
  updateOpenClawDiscordAccountConfig,
  updateOpenClawDiscordConfig,
  validateOpenClawDiscordCredential,
} from '@/api'
...
type DiscordAddForm = {
  ...
  actionModeration: boolean
  actionPermissions: boolean
  actionPins: boolean
  actionPolls: boolean
  actionReactions: boolean
  actionStickers: boolean
  actionThreads: boolean
  accountId: string
  agentId: string
  allowFrom: string
...
const discordHelpSteps: Array<{ content: ReactNode; step: string }> = [
  {
    step: '1',
    content: (
      <>
        ...
        ，创建 Application 和 Bot，并复制 Bot Token。
      </>
    ),
  },
  { step: '2', content: '在 OAuth2 URL Generator 勾选 bot 和 applications.commands，并把 Bot 邀请到你的服务器。' },
  { step: '3', content: '复制 Application ID 和 Server ID，可在账号信息里一并写入。' },
  { step: '4', content: '保持 DM 策略为 pairing，启动网关后向 Bot 发私信，再在这里批准 pairing code。' },
  {
    step: '5',
    content: (
      <>
        参考教程：{' '}
        <Link href="https://zhuanlan.zhihu.com/p/2001934699661137919" target="_blank" rel="noreferrer">
          知乎教程
          <Link.Icon />
        </Link>
        {' · '}
        <Link href="https://cloud.tencent.com/developer/article/2626068" target="_blank" rel="noreferrer">
          腾讯云教程
          <Link.Icon />
        </Link>
      </>
...
const defaultAddForm: DiscordAddForm = {
  actionMessages: true,
  actionModeration: false,
...
export function OpenClawDiscordPanel() {
  const [state, setState] = useState<LoadState>('idle')
  const [status, setStatus] = useState<OpenClawDiscordStatusResponse | null>(null)
  const [accountDrafts, setAccountDrafts] = useState<Record<string, DiscordAccountDraft>>({})
  ...
  const [taskKind, setTaskKind] = useState<DiscordTaskKind | null>(null)
  ...
  const [addForm, setAddForm] = useState<DiscordAddForm>(defaultAddForm)
  const [accountEditTarget, setAccountEditTarget] = useState<OpenClawDiscordAccount | null>(null)
  const [accountDeleteTarget, setAccountDeleteTarget] = useState<OpenClawDiscordAccount | null>(null)
  ...
  const loadStatus = useCallback(async () => {
    ...
    setError('')
    try {
      const [nextStatus, nextAgents] = await Promise.all([getOpenClawDiscordStatus(), listOpenClawAgents()])
      setStatus(nextStatus)
      setAgents(nextAgents.agents ?? [])
      setAccountDrafts(buildDiscordAccountDrafts(nextStatus.accounts ?? []))
      setState('ready')
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Discord 渠道状态加载失败')
      setState('error')
    }
  }, [])
  ...
  const startStream = useCallback((kind: DiscordTaskKind, url: string) => {
    closeStream()
    ...
    source.addEventListener('log', (event) => {
      try {
        ...
        setTask((current) => current ? {
          ...current,
          id: payload.id || current.id,
          logs: appendTaskLog(current.logs, maskDiscordToken(payload.line)),
          updatedAt: payload.timestamp,
        } : current)
      } catch {
    ...
    source.onerror = () => {
      ...
      toast.warning('Discord 渠道流式任务连接中断')
      void loadStatus()
    }
  ...
  const submitEditAccount = useCallback(async () => {
    ...
    try {
      await updateOpenClawDiscordConfig(buildDiscordAdvancedConfigRequest(addForm))
      const nextStatus = await updateOpenClawDiscordAccountConfig(accountEditTarget.accountId, {
        actions: {
          messages: addForm.actionMessages,
          moderation: addForm.actionModeration,
          permissions: addForm.actionPermissions,
          pins: addForm.actionPins,
          polls: addForm.actionPolls,
          reactions: addForm.actionReactions,
          stickers: addForm.actionStickers,
          threads: addForm.actionThreads,
        },
        allowFrom: splitDiscordAccountFormList(addForm.allowFrom),
        applicationId: addForm.applicationId.trim(),
        dmPolicy: addForm.dmPolicy,
        execApprovals: {
          approvers: splitDiscordAccountFormList(addForm.execApprovalsApprovers),
        ...
        guildIds: splitDiscordAccountFormList(addForm.guildIds),
      ...
      setStatus(nextStatus)
      setAccountDrafts(buildDiscordAccountDrafts(nextStatus.accounts ?? []))
      setAddForm(buildDiscordAccountFormFromAccount((nextStatus.accounts ?? []).find((item) => item.accountId === accountEditTarget.accountId) ?? accountEditTarget, nextStatus.config))
      setIsAddOpen(false)
      setAccountEditTarget(null)
      toast.success('Discord 账号已更新')
    } catch (err) {
      toast.warning(err instanceof Error ? err.message : 'Discord 账号更新失败')
    } finally {
      setSavingAccountId('')
    }
  ...
  const submitAddAccount = useCallback(() => {
    ...
    setIsAddOpen(false)
    startStream('add', getOpenClawDiscordAddAccountStreamURL(addForm))
  ...
  const openEditAccountModal = useCallback((account: OpenClawDiscordAccount) => {
    setAccountModalMode('edit')
    setAccountEditTarget(account)
    setAddForm(buildDiscordAccountFormFromAccount(account, status?.config))
    setActiveAddSection('form')
    setIsAddOpen(true)
  }, [status?.config])
  ...
  const updateChannelEnabled = useCallback(async (enabled: boolean) => {
    ...
    try {
      const nextStatus = await updateOpenClawDiscordConfig({ enabled })
      setStatus(nextStatus)
      setAccountDrafts(buildDiscordAccountDrafts(nextStatus.accounts ?? []))
      toast.success(enabled ? 'Discord 渠道已启用' : 'Discord 渠道已停用')
    } catch (err) {
      void loadStatus()
      toast.warning(err instanceof Error ? err.message : 'Discord 渠道开关更新失败')
    } finally {
  ...
  const updateChannelDMPolicy = useCallback(async (dmPolicy: string) => {
    ...
    try {
      const nextStatus = await updateOpenClawDiscordConfig({ dmPolicy })
      setStatus(nextStatus)
      setAccountDrafts(buildDiscordAccountDrafts(nextStatus.accounts ?? []))
  ...
  const updateAccount = useCallback(async (account: OpenClawDiscordAccount, patch: Partial<DiscordAccountDraft>) => {
    const draft = {
      ...getDiscordAccountDraft(accountDrafts, account.accountId, account),
      ...patch,
    }
    setAccountDrafts((current) => ({ ...current, [account.accountId]: draft }))
    setSavingAccountId(account.accountId)
    try {
      const nextStatus = await updateOpenClawDiscordAccountConfig(account.accountId, {
        agentId: draft.agentId,
        enabled: draft.enabled,
      })
      setStatus(nextStatus)
      setAccountDrafts(buildDiscordAccountDrafts(nextStatus.accounts ?? []))
      toast.success('Discord 账号配置已更新')
    } catch (err) {
      setAccountDrafts(buildDiscordAccountDrafts(status?.accounts ?? []))
      toast.warning(err instanceof Error ? err.message : 'Discord 账号配置更新失败')
    } finally {
      setSavingAccountId('')
    }
  ...
  const deleteAccount = useCallback(async () => {
    ...
    try {
      await deleteOpenClawDiscordAccount(accountDeleteTarget.accountId)
      setAccountDeleteTarget(null)
      toast.success('Discord 账号已删除')
      void loadStatus()
    } catch (err) {
      toast.warning(err instanceof Error ? err.message : 'Discord 账号删除失败')
    } finally {
      ...
      {error ? (
        <div className=" py-3 w-full">
          <Alert status="danger" className='items-center'>
            <Alert.Indicator />
            <Alert.Content>
              <Alert.Title>Discord渠道状态加载失败</Alert.Title>
              <Alert.Description>
                请检查 Discord 渠道状态和网关运行状态。
              </Alert.Description>
            </Alert.Content>
      ...
      <div className="flex flex-col gap-2  w-full">
            ...
            <div className="flex w-full items-center justify-between gap-4">
              <div className="flex min-w-0 items-start gap-3">
                <div className="flex size-11 shrink-0 items-center justify-center rounded-full bg-accent/10 text-accent">
                  <Icon icon="ri:discord-fill" className="size-7" />
                </div>
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2">
                {state !== 'loading' && !isTaskRunning ? (
                  <Button size="sm" isIconOnly variant={state === 'error' ? 'primary' : 'ghost'} aria-label="刷新 Discord 渠道状态" onPress={() => void loadStatus()}>
        ...
        {configured ? (
          accounts.length > 0 ? (
            <div className="grid gap-2 lg:grid-cols-2">
              {accounts.map((account) => (
                <DiscordAccountCard
                  ...
                  draft={getDiscordAccountDraft(accountDrafts, account.accountId, account)}
            ...
            <ChannelEmptyState
              description="添加一个 Discord Bot 账号后，就可以配置路由 Agent、启用状态和消息接入策略。"
              icon="ri:discord-fill"
              title="还没有 Discord 账号"
            />
          )
        ) : (
          <ChannelEmptyState
            description="配置 Bot Token 后，可以在这里管理 Discord 账号和消息接入。"
          ...
          <AlertDialog.Container>
            <AlertDialog.Dialog className="sm:max-w-[480px]">
              ...
              <AlertDialog.Header>
                ...
                <AlertDialog.Heading>删除 Discord 账号？</AlertDialog.Heading>
              </AlertDialog.Header>
              <AlertDialog.Body>
...
function DiscordAccountCard({
  ...
  isDisabled: boolean
  isSaving: boolean
  onChange: (patch: Partial<DiscordAccountDraft>) => void
  onDelete: (account: OpenClawDiscordAccount) => void
  onEdit: (account: OpenClawDiscordAccount) => void
}) {
      ...
      <Card.Content>
        <div className="flex gap-4 items-center justify-between">
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            ...
            <span className="h-5 w-px shrink-0 bg-border" aria-hidden="true" />
            <Button isIconOnly size="sm" variant="tertiary" aria-label="删除 Discord 账号" onPress={() => onDelete(account)} isDisabled={isDisabled || isSaving}>
...
function DiscordAddAccountModal({
  activeSection,
  agentOptions,
  editAccount,
  form,
  ...
  agentOptions: AgentOption[]
  editAccount: OpenClawDiscordAccount | null
  form: DiscordAddForm
  isOpen: boolean
  isSubmitting: boolean
  mode: AccountModalMode
  onActiveSectionChange: (section: AddFormSection) => void
  onFormChange: (form: DiscordAddForm) => void
  onOpenChange: (open: boolean) => void
  onSubmit: () => void
}) {
  const update = (patch: Partial<DiscordAddForm>) => onFormChange({ ...form, ...patch })
  const [credentialCheck, setCredentialCheck] = useState<DiscordCredentialCheck | null>(null)
  ...
  const validateCredential = async () => {
    ...
    try {
      const result = await validateOpenClawDiscordCredential({ botToken: form.botToken.trim() })
      if (result.valid) {
        const label = result.username ? `@${result.username}` : String(result.botId ?? '')
        setCredentialCheck({ label, signature: credentialSignature, valid: true })
        toast.success(label ? `Discord Bot 校验通过：${label}` : 'Discord Bot 校验通过')
      } else {
        setCredentialCheck({ error: result.error || 'Discord Bot Token 校验失败', signature: credentialSignature, valid: false })
        toast.warning(result.error || 'Discord Bot Token 校验失败')
      }
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Discord Bot Token 校验失败'
      ...
      <Modal.Container size="lg" scroll="inside">
        <Modal.Dialog className="sm:max-w-[680px]">
          ...
          <Modal.Header>
            ...
            <div>
              <Modal.Heading>{mode === 'edit' ? '编辑 Discord 账号' : '添加 Discord 账号'}</Modal.Heading>
              <p className="mt-1 text-sm text-muted">
                {mode === 'edit'
                  ? `更新 ${editAccount?.name || editAccount?.accountId || 'Discord 账号'} 的基础和高级设置。`
                  : '填写 Developer Portal token 后写入 Discord 渠道。'}
              </p>
            </div>
          </Modal.Header>
          <Modal.Body>
            <div className="space-y-5 p-1">
              ...
              {activeSection === 'help' ? (
                ...
                <div className="space-y-5 max-h-[500px] overflow-y-auto">
                  ...
                  <TextareaCard
                    description="支持逗号或换行分隔；Allowlist 策略至少需要一个 Discord 用户 ID。"
                    disabled={isSubmitting}
                    icon="lucide:users"
                    placeholder="discord:123456789012345678, discord:987654321098765432"
                    title="准入用户"
                  ...
                  <TextareaCard
                    description="支持逗号或换行分隔；Guild 策略为 allowlist 时，只有这些服务器会接入。"
                    disabled={isSubmitting}
                    icon="lucide:server"
                    placeholder="123456789012345678"
                    title="服务器 ID"
                    value={form.guildIds}
                    onChange={(guildIds) => update({ guildIds })}
                  />

                  <ItemCardGroup className="overflow-hidden">
                    <ItemCardGroup.Header>
                      <ItemCardGroup.Title>Exec 审批</ItemCardGroup.Title>
                      <ItemCardGroup.Description>配置 Discord 原生审批提示和审批者。</ItemCardGroup.Description>
                    </ItemCardGroup.Header>
                  ...
                  <TextareaCard
                    description="支持逗号或换行分隔。审批者必须是数字 Discord 用户 ID。"
                    disabled={isSubmitting}
                  ...
                  <ItemCardGroup className="overflow-hidden">
                    <ItemCardGroup.Header>
                      ...
                      <ItemCardGroup.Description>控制 Discord 预览消息、工具进度和命令文本显示。</ItemCardGroup.Description>
                    </ItemCardGroup.Header>
                  ...
                  <ItemCardGroup className="overflow-hidden">
                    ...
                    <FormItem description="Discord setMyCommands 的内置命令。" icon="lucide:command" title="原生命令">
                      <FriendlySelect
                        ariaLabel="原生命令"
                        isDisabled={isSubmitting}
                        options={booleanModeOptions}
                  ...
                  <ItemCardGroup className="overflow-hidden">
                    <ItemCardGroup.Header>
                      ...
                      <ItemCardGroup.Description>控制 Discord 消息、表情、线程和管理动作。</ItemCardGroup.Description>
                    </ItemCardGroup.Header>
                    <ActionSwitch label="消息" value={form.actionMessages} disabled={isSubmitting} onChange={(actionMessages) => update({ actionMessages })} />
                    <ActionSwitch label="表情回应" value={form.actionReactions} disabled={isSubmitting} onChange={(actionReactions) => update({ actionReactions })} />
                    <ActionSwitch label="贴纸" value={form.actionStickers} disabled={isSubmitting} onChange={(actionStickers) => update({ actionStickers })} />
                    <ActionSwitch label="投票" value={form.actionPolls} disabled={isSubmitting} onChange={(actionPolls) => update({ actionPolls })} />
                    <ActionSwitch label="线程" value={form.actionThreads} disabled={isSubmitting} onChange={(actionThreads) => update({ actionThreads })} />
                    <ActionSwitch label="置顶" value={form.actionPins} disabled={isSubmitting} onChange={(actionPins) => update({ actionPins })} />
                    <ActionSwitch label="权限" value={form.actionPermissions} disabled={isSubmitting} onChange={(actionPermissions) => update({ actionPermissions })} />
                    <ActionSwitch label="管理动作" value={form.actionModeration} disabled={isSubmitting} onChange={(actionModeration) => update({ actionModeration })} />
                  </ItemCardGroup>
                </div>
              ) : (
                <div className="space-y-5 max-h-[500px] overflow-y-auto">
                  <ItemCardGroup className="overflow-hidden">
                    <ItemCardGroup.Header>
                      ...
                      <ItemCardGroup.Description>{mode === 'edit' ? '编辑账号名称与基础标识。' : '填写 Discord Developer Portal 返回的账号凭据。'}</ItemCardGroup.Description>
                    </ItemCardGroup.Header>
                    ...
                    <Separator />
                    <FormItem description={mode === 'edit' ? '留空保持当前 Bot Token 不变。' : '从 Developer Portal 获取。'} icon="lucide:key-round" title="Bot Token">
                      <ClearableInput value={form.botToken} placeholder="Bot Token" disabled={isSubmitting} onChange={(botToken) => update({ botToken })} />
                    </FormItem>
                    <Separator />
                    <FormItem description="可选；网络受限或多账号时建议填写 Application ID。" icon="lucide:fingerprint" title="Application ID">
                      <ClearableInput value={form.applicationId} placeholder="123456789012345678" disabled={isSubmitting} onChange={(applicationId) => update({ applicationId })} />
                    </FormItem>
                    {mode === 'create' ? (
                      <>
                        <Separator />
                  ...
                  {mode === 'create' ? (
                    <ItemCardGroup className="overflow-hidden">
                      <ItemCardGroup.Header>
                        ...
                        <ItemCardGroup.Description>控制谁可以通过 Discord 私聊或群聊唤起 OpenClaw。</ItemCardGroup.Description>
                      </ItemCardGroup.Header>
                      ...
                      <FormItem description="服务器频道准入策略。" icon="lucide:messages-square" title="Guild 策略">
                        <FriendlySelect
                          ariaLabel="Guild 策略"
                          isDisabled={isSubmitting}
                          options={groupPolicyOptions}
                  ...
                  {mode === 'create' ? (
                    <TextareaCard
                      description="支持逗号或换行分隔；留空时只完成 DM 配置。"
                      disabled={isSubmitting}
                      icon="lucide:server"
                      placeholder="123456789012345678"
                      title="服务器 ID"
          ...
          <Modal.Footer>
            ...
            {canValidateCredential ? (
              <Button variant="tertiary" isPending={isCredentialChecking} isDisabled={isSubmitting} onPress={() => void validateCredential()}>
            ...
            <Button
              variant="tertiary"
              onPress={() => void openExternalUrl('https://discord.com/developers/applications')}
            >
...
function TaskLogPanel({ logRef, onClose, task, taskKind }: { logRef: RefObject<HTMLPreElement | null>; onClose: () => void; task: OpenClawChannelTaskResponse | null; taskKind: DiscordTaskKind | null }) {
  if (!task) return null
...
function buildDiscordAccountDrafts(accounts: OpenClawDiscordAccount[]) {
  return accounts.reduce<Record<string, DiscordAccountDraft>>((drafts, account) => {
    drafts[account.accountId] = getDiscordAccountDraft({}, account.accountId, account)
    return drafts
  }, {})
}

function buildDiscordAccountFormFromAccount(account: OpenClawDiscordAccount, config?: OpenClawDiscordConfig): DiscordAddForm {
  return {
    actionMessages: booleanDefault(account.actions?.messages, config?.actions?.messages, true),
    actionModeration: booleanDefault(account.actions?.moderation, config?.actions?.moderation, false),
    actionPermissions: booleanDefault(account.actions?.permissions, config?.actions?.permissions, false),
    actionPins: booleanDefault(account.actions?.pins, config?.actions?.pins, true),
    actionPolls: booleanDefault(account.actions?.polls, config?.actions?.polls, true),
    actionReactions: booleanDefault(account.actions?.reactions, config?.actions?.reactions, true),
    actionStickers: booleanDefault(account.actions?.stickers, config?.actions?.stickers, true),
    actionThreads: booleanDefault(account.actions?.threads, config?.actions?.threads, true),
    ...
    guildIds: (account.guildIds ?? []).join('\n'),
    name: account.name || '',
...
function buildDiscordAdvancedConfigRequest(form: DiscordAddForm): OpenClawDiscordConfigUpdateRequest {
  return {
    actions: {
      messages: form.actionMessages,
      moderation: form.actionModeration,
      permissions: form.actionPermissions,
      pins: form.actionPins,
      polls: form.actionPolls,
      reactions: form.actionReactions,
      stickers: form.actionStickers,
      threads: form.actionThreads,
    },
    commands: {
    ...
    execApprovals: {
      approvers: splitDiscordAccountFormList(form.execApprovalsApprovers),
...
function splitDiscordAccountFormList(value: string) {
  return value
    .split(/[,，\n\r;；]/)
    .map((item) => item.trim())
    .filter(Boolean)
...
function getDiscordAccountDraft(drafts: Record<string, DiscordAccountDraft>, accountId: string, account?: OpenClawDiscordAccount): DiscordAccountDraft {
...
function maskDiscordToken(line: string) {
  return line.replace(/\b\d{5,}:[A-Za-z0-9_-]{12,}\b/g, '<bot-token>')
}

function taskTitle(kind: DiscordTaskKind | null) {
  switch (kind) {
    case 'add':
      return '添加 Discord 账号'
    default:
      return 'Discord 渠道任务'
  }
}

function OpenClawDiscordPage() {
  usePageTitle('OpenClaw Discord 渠道')

  return (
    <DashboardLayout>
      <div className="mx-auto flex max-w-7xl flex-col gap-6">
        <OpenClawDiscordPanel />
      </div>
    </DashboardLayout>
  )
}

export default OpenClawDiscordPage
