...
import type { HermesAgentInfo, HermesTerminalCommand, HermesTerminalSession } from '@/api'
import { createHermesTerminal, deleteHermesTerminalRecord, getHermesTerminalWebSocketURL, listHermesTerminals, stopHermesTerminal } from '@/api'
...
type CommandOption = { value: HermesTerminalCommand; label: string; description: string; icon: string }
...
const commandOptions: CommandOption[] = [
  { value: 'chat', label: 'Chat', description: '启动 hermes chat 交互会话', icon: 'lucide:message-square-text' },
  { value: 'chat-tui', label: 'TUI Chat', description: '启动 hermes chat --tui', icon: 'lucide:panel-top' },
  { value: 'continue', label: 'Continue', description: '继续最近一次 Hermes 会话', icon: 'lucide:history' },
  { value: 'setup', label: 'Setup', description: '进入 Hermes setup 向导', icon: 'lucide:settings-2' },
]

const terminalRouteLog = (message: string, detail?: Record<string, unknown>) => {
  console.info(`[Hermes终端路由] ${message}`, detail ?? '')
}

function HermesTerminalPage() {
  usePageTitle('Hermes 终端')
  ...
  const [command, setCommand] = useState<HermesTerminalCommand>('chat-tui')
  ...
  const [deleteRecordTarget, setDeleteRecordTarget] = useState<HermesTerminalSession | null>(null)
  ...
  const detachedProfileName = detachedSearchParams.get('profile') ?? ''
  ...
  const sessionsRef = useRef<HermesTerminalSession[]>([])
  const currentSessionRef = useRef<HermesTerminalSession | null>(null)
  const attachedSessionIdRef = useRef('')
  const currentSession = sessions.find((item) => item.id === activeSessionId) ?? null

  const selectedProfileName = detachedProfileName || selectedAgentName || selectedProfile?.name || 'default'

  useEffect(() => {
    terminalRouteLog('Hermes Terminal 页面已挂载', {
    ...
    return () => {
      terminalRouteLog('Hermes Terminal 页面开始卸载')
    }
  }, [detachedInitialSessionId, detachedProfileName, isDetachedTerminalWindow, selectedProfileName])

  const loadSessions = useCallback(async () => {
    setState('loading')
    setError('')
    try {
      const payload = await listHermesTerminals(selectedProfileName)
      ...
      setError(err instanceof Error ? err.message : 'Hermes 终端会话加载失败')
      setState('error')
    }
  }, [detachedInitialSessionId, selectedProfileName])
  ...
  const attachSession = useCallback((session: HermesTerminalSession, force = false) => {
    const terminal = terminalRef.current
    if (!terminal) return
    ...
    const socket = new WebSocket(getHermesTerminalWebSocketURL(session.id))
  ...
  const startSession = useCallback(async (options?: { command?: HermesTerminalCommand; cwd?: string }) => {
    const launchCommand = options?.command ?? command
    const launchCwd = options?.cwd ?? cwd
    setIsStarting(true)
    try {
      const terminal = terminalRef.current
      const created = await createHermesTerminal({
      ...
      setDetachedSessionId('')
      setLaunchOpen(false)
      toast.success('Hermes 终端已启动')
    } catch (err) {
      toast.warning(err instanceof Error ? err.message : 'Hermes 终端启动失败')
    } finally {
      setIsStarting(false)
    }
  }, [command, cwd, selectedProfileName])

  const stopSession = useCallback(async (session: HermesTerminalSession) => {
    setIsStopping(true)
    try {
      const stopped = await stopHermesTerminal(session.id)
      setSessions((current) => upsertTerminalSession(current, stopped))
      toast.success('Hermes 终端已停止')
    } catch (err) {
  ...
  const deleteSessionRecord = useCallback(async () => {
    ...
    try {
      await deleteHermesTerminalRecord(session.id)
  ...
  const openTerminalWindow = useCallback(async () => {
    ...
    url.searchParams.set('profile', currentSession.profile.name || selectedProfileName)
    const popup = window.open(url.toString(), `hermes-terminal-${currentSession.id}`, 'popup=yes,width=960,height=640,left=120,top=90,resizable=yes,scrollbars=no')
    if (!popup) {
    ...
    socketRef.current?.close()
    terminalRef.current?.clear()
  }, [currentSession, selectedProfileName])
            ...
            <div className="flex w-full flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <Card.Title>{currentSession?.command || `${selectedProfileName} ${activeCommand.label}`}</Card.Title>
                  <Chip color={connectionTone.color} size="sm" variant="soft">
                    <Chip.Label>{connectionTone.label}</Chip.Label>
                  </Chip>
                </div>
                <Card.Description className="truncate">{currentSession?.cwd || activeCommand.description}</Card.Description>
              </div>
            </div>
            {error ? <div className="mt-3 rounded-xl bg-danger/10 px-3 py-2 text-sm text-danger">{error}</div> : null}
          </Card.Header> */}
          <div className="relative min-h-0 flex-1 p-0">
            ...
            {shouldShowMainEmptyState ? (
              <TerminalEmptyState
                ...
                onQuickStart={() => void startSession({ command: 'chat-tui', cwd: '' })}
              />
            ) : null}
            ...
            <AlertDialog.Backdrop isOpen={deleteRecordTarget !== null} onOpenChange={(open) => {
              ...
              <AlertDialog.Container>
                <AlertDialog.Dialog className="sm:max-w-[460px]">
                  ...
                  <AlertDialog.Body>
                    这会删除终端会话「{deleteRecordTarget ? formatSessionTitle(deleteRecordTarget) : '-'}」在 {appName} SQLite 中的记录，不会删除 Hermes 会话内容。
                  </AlertDialog.Body>
                  <AlertDialog.Footer>
    ...
    <Card className="w-full h-full items-center">
      <Card.Content className="flex h-full w-full flex-col items-center justify-center text-center">
        ...
        <div className="mt-2 max-w-sm leading-6 text-muted text-sm mb-4">
          选择 Agent、命令和工作目录后启动一个 Hermes 终端，会话输出会显示在这里。
        </div>
...
function LaunchTerminalModal({
  ...
  onCommandChange: (command: HermesTerminalCommand) => void
      ...
      <Modal.Container size="md">
        <Modal.Dialog>
          ...
          <Modal.Header>
            ...
            <div>
              <Modal.Heading>新建 Hermes 终端</Modal.Heading>
              <p className="mt-1 text-sm text-muted">选择 Agent、启动命令和工作目录。</p>
            </div>
          </Modal.Header>
          <Modal.Body>
            <div className="grid gap-4">
              <ItemCardGroup className="overflow-hidden">
                <ItemCardGroup.Header>
                  ...
                  <ItemCardGroup.Description>选择运行 Hermes 终端使用的 Profile 和命令。</ItemCardGroup.Description>
                </ItemCardGroup.Header>
                <TerminalFormItem icon="lucide:brain" title="Agent">
                  <ProfileSelectDropdown profiles={profiles} selectedName={selectedProfileName} onSelect={onProfileChange} />
                </TerminalFormItem>
                <div className="h-px bg-separator" />
                <TerminalFormItem icon={selectedCommand.icon} title="命令">
                  <CellSelect aria-label="启动命令" value={command} variant="secondary" onChange={(key: Key | null) => key && onCommandChange(String(key) as HermesTerminalCommand)}>
                    <CellSelect.Trigger>
                      <CellSelect.Value>
                        {() => (
                ...
                <Card.Header>
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <Card.Description>留空时使用当前 Profile 的默认工作目录。</Card.Description>
                    </div>
                  </div>
                </Card.Header>
                <Card.Content>
                  <Input
                    aria-label="工作目录"
                    placeholder={selectedProfile?.workspaceDir || selectedProfile?.path || '默认 Profile 工作目录'}
                    value={cwd}
          ...
          <Modal.Footer>
            ...
            <Button variant="primary" isPending={isStarting} onPress={onStart}>
              {({ isPending }) => (
                <>
...
function SessionStatusAvatar({ session, size = 'md' }: { session: HermesTerminalSession; size?: 'sm' | 'md' }) {
  const sizeClass = size === 'sm' ? 'size-7 [&>svg]:size-3.5' : 'size-9 [&>svg]:size-4'
...
function formatSessionTitle(session: HermesTerminalSession) {
...
function upsertTerminalSession(sessions: HermesTerminalSession[], next: HermesTerminalSession) {
...
function isTerminalSessionInteractive(session: HermesTerminalSession | null) {
