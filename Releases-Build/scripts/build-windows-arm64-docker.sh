...
IMAGE_NAME="${IMAGE_NAME:-agentbox-windows-arm64-builder}"
DOCKERFILE="$ROOT_DIR/Build/docker/windows-arm64/Dockerfile"
...
docker run --rm \
  ...
  -v agentbox-windows-arm64-client-node-modules:/workspace/Client/node_modules \
  -w /workspace \
  "$IMAGE_NAME"
